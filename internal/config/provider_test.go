...
func TestServerParameters_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		{"only-username", &ServerParameters{BindAddress: "0.0.0.0", BindPort: 2022, PortRangeStart: 1000, PortRangeEnd: 2000, Username: "user", Password: "", PrivateRsaPath: filepath.Join(tempDir, "/id_rsa")}, false, ""},
		{"only-password", &ServerParameters{BindAddress: "0.0.0.0", BindPort: 2022, PortRangeStart: 1000, PortRangeEnd: 2000, Username: "", Password: "pass", PrivateRsaPath: filepath.Join(tempDir, "/id_rsa")}, false, ""},
